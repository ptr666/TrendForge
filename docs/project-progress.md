...
| 3 | Verification failure creates BrowserAct planned command and diagnosable run events | done | Pipeline tests prove selected HTTP source items create `fetch_full_text` BrowserAct planned events and that BrowserAct-acquired full text feeds summaries and drafts | Keeps difficult pages observable and safe by default; real browser automation is plugged through `FullTextProvider`, and MediaCrawler remains gated behind explicit enablement. |
...
| 5 | WeChat draft maps to the `wechat-official-account-workflow` skill contract | done | Dry-run publish results and run events include queued WeChat `plannedCommands` for preview, check, and create-draft plus `state/published.json` / preview verification signal | Uses `trendforge-adapter-contract`; real draft creation remains gated by explicit approval, credentials, and IP whitelist readiness. |
| 6 | XHS draft maps to the `xhs-browser-draft-setup` skill workflow | done | Dry-run publish results and run events include queued XHS `plannedCommands` for check-login, fill-publish, and save-draft plus browser `draft-saved` verification signal | Uses `trendforge-adapter-contract`; real draft save remains gated by explicit approval, Hermes/bridge/extension/login health. |
| 7 | CLI/API can query run history, items, and drafts | done | CLI and API acceptance tests run RSS pipelines with stable run ids, then read back runs, events, items, and drafts through public surfaces | Makes local operation inspectable; `TRENDFORGE_RUNS_DIR` isolates test and experiment history. |
