...
| 7 | CLI/API can query run history, items, and drafts | done | CLI and API acceptance tests run RSS pipelines with stable run ids, including the committed RSS fixture via `--query-file`, then read back runs, events, items, and drafts through public surfaces | Makes local operation inspectable; `TRENDFORGE_RUNS_DIR` isolates test and experiment history. |
