...
| 5 | WeChat draft maps to the `wechat-official-account-workflow` skill contract | done | Dry-run publish results include a queued WeChat draft plan and `state/published.json` / preview verification signal | Uses `trendforge-adapter-contract`; real draft creation remains gated by explicit approval, credentials, and IP whitelist readiness. |
| 6 | XHS draft maps to the `xhs-browser-draft-setup` skill workflow | done | Dry-run publish results include a queued XHS draft plan and browser `draft-saved` verification signal | Uses `trendforge-adapter-contract`; real draft save remains gated by explicit approval, Hermes/bridge/extension/login health. |
| 7 | CLI/API can query run history, items, and drafts | done | CLI acceptance test runs an RSS pipeline with `--run-id`, then reads it back through `trendforge runs` and `trendforge events`; API exposes matching run/item/draft surfaces | Makes local operation inspectable; `TRENDFORGE_RUNS_DIR` isolates test and experiment history. |
