...
| 10 | Browser workbench begins after the pipeline is inspectable | planned | `apps/web` exposes the first useful local workflow backed by existing API/run state | Do not start before earlier slices are usable. |
