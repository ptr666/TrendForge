...
| 3 | Verification failure creates BrowserAct planned command and diagnosable run events | done | Pipeline tests prove selected HTTP source items create `fetch_full_text` BrowserAct planned events without executing real browser automation | Keeps difficult pages observable and safe by default; MediaCrawler remains gated behind explicit enablement. |
