...
默认安全策略不变：可以生成本地草稿和平台 handoff，但不会正式发布；真实创建微信公众号或小红书草稿必须显式开启并通过对应 gate。图片生成模型未单独配置时，系统不会申请图片生成；图片预览和单图重生成在草稿页完成，不再走单独审批流程。
...
微信公众号只输入 AppID 可以保存本地配置，但不能完成上传或创建草稿。真实公众号草稿创建需要 AppID、AppSecret、服务器 IP 白名单通过 token 检查，并提供封面 media ID 或本地封面路径。若填写本地封面路径，后端会使用微信官方 API，先通过 `/cgi-bin/material/add_material` 上传为永久素材并取得 `thumb_media_id`；本地 Markdown 草稿会在上传前转换为带基础样式的微信 HTML，草稿正文里的远程或本地图片会通过 `/cgi-bin/media/uploadimg` 转存到微信图床，最后调用 `/cgi-bin/draft/add` 创建草稿。
...
- 单条评分、原文或总结失败不会中断整轮分析。评分失败的条目会记 0 分并跳过；原文不可用的条目会写入 skipped 原因；总结模型失败时会生成中文兜底摘要并标记为 fallback。
- 如果可用条目足够，系统会继续向后补足用户设置的候选数量。
...
草稿生成是长任务，页面会轮询 `GET /runs/:runId/events` 并显示当前阶段、已处理数量和耗时。草稿生成的进度只读取 `draft_generation` 之后的事件，因此热点分析阶段的“原文正文太短”等提醒不会在草稿生成阶段重复显示。
...
- 未配置图片 provider：不生成图片资产、不生成图片 prompt。
...
- 成功生成的图片状态为 `ready`；失败或缺配置时状态为 `failed` 或 `blocked`，只作为异常提醒展示。
...
“阻塞与提醒”不是主流程入口，也不承载审批流程，只展示异常情况：
...
正常生成的图片不会进入“阻塞与提醒”。如果需要调整图片，请在草稿生成区域的图片资产栏查看 prompt、预览图片或单独重生成。

原文获取失败类提醒只用于候选生成和热点分析诊断。若某条信息因为“HTTP 原文获取失败：抽取出的正文太短”被跳过或记为 0 分，草稿生成阶段不再重复展示这条提醒。
